...
export const globalCompletions = globalVariables.map(
  (globalVariable) => completions[globalVariable],
);

export const getGlobalCompletionsByKeyword = (keyword?: string) => {
  if (keyword === undefined) {
    const gc = globalVariables.map((globalVariable) => completions[globalVariable]);
    return gc;
  }
